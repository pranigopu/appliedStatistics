...
**NOTE 2: The range of CDP and PDF**:<br>CDF outputs the cumulative probability mass of a point, which has to be in the interval $[0, 1]$ by the nature of probability. However, PDF being the _gradient_ of the cumulative probability mass at a point is not bound to $[0, 1]$. But by the nature of cumulative probabilities (which can only grow for higher values, never decrease), the gradient of the CDF and thus the PDF can never be negative, which means the range of the PDF is $[0, \infty)$.
